...
cfg.scene_type = '3D';             % '1D' or '3D'

% Sun-view geometry
cfg.geometry.SZA = 20;             % Solar zenith angle [deg]
cfg.geometry.SAA = 0;              % Solar azimuth angle [deg]
cfg.geometry.view_angles = [];     % Optional [VZA, VAA] grid [deg]; [] uses default multi-angle
...
cfg.lad.LIDFa = 59;                % Campbell mean leaf angle [deg]
...
cfg.canopy.leaf_diameter = 0.005;   % Crown-scale hotspot leaf-size parameter [m]

cfg.canopy.use_external_crown_center = true;
cfg.canopy.external_crown_center = 0.5;
cfg.numerics.CI_min = 0.005;        % 3D effective CI lower bound
